...
def convert_to_image(base64_code):
    ...
    if result_image is None:
        logger.error("this encode cannot convert to image,cause :convert_to_image")
        return "error"
    else:
        logger.info("decoding success,return the result_image")
        return result_image
...
def contrast_faces(known_image, unknown_image):
    ...
    try:
        ...
        logger.info("contrast faces success,return a bool value")
        return result[0]
    except IndexError:
        logger.error("cannot find faces in image,cause:contrast_faces")
        return "nf"
    except Exception as e:
        logger.error("recognition error,cause:{}".format(e))
